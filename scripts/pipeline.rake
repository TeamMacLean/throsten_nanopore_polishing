...
file "results/#{@sample}/#{@sample}_pilon_polished_round1.fasta" => "results/#{@sample}/illumina_aligned_to_assembly_sorted.sorted.bam" do
    ...
    sh "source pilon-1.22;  pilon --genome results/#{@sample}/racon_round2_output.fasta --unpaired results/#{@sample}/illumina_aligned_to_assembly_sorted.sorted.bam --outdir results/#{@sample}/ --output #{@sample}_pilon_polished_round1 --changes --threads 16 "
end
...
file "results/#{@sample}/#{@sample}_pilon_polished_round2.fasta" => [ "results/#{@sample}/illumina_aligned_to_assembly_sorted2.sorted.bam"] do
    sh "source pilon-1.22;  pilon --genome results/#{@sample}/#{@sample}_pilon_polished_round1.fasta --unpaired results/#{@sample}/illumina_aligned_to_assembly_sorted2.sorted.bam --outdir results/#{@sample} --output #{@sample}_pilon_polished_round2 --changes --threads 16"
end

#pilon round 1 for CD156
file "results/#{@sample}/#{@sample}_illumina_aligned_to_assembly_sorted.sorted.bam" => ["results/#{@sample}/racon_round2_output.fasta.1.bt2", "#{@R1}" ] do
    puts "running bowtie2 for pilon round 1 for CD156 "
    sh "source bowtie2-2.2.9; source samtools-1.9;  bowtie2 --threads 2 --qc-filter --no-unal -x results/#{@sample}/racon_round2_output.fasta -U #{@R1}  -S results/#{@sample}/illumina_aligned_to_assembly1.sam &&  samtools view -b results/#{@sample}/illumina_aligned_to_assembly1.sam | samtools sort -o results/#{@sample}/illumina_aligned_to_assembly_sorted.sorted.bam && ln -s results/#{@sample}/illumina_aligned_to_assembly_sorted.sorted.bam results/#{@sample}/#{@sample}_illumina_aligned_to_assembly_sorted.sorted.bam && samtools index results/#{@sample}/#{@sample}_illumina_aligned_to_assembly_sorted.sorted.bam && rm results/#{@sample}/illumina_aligned_to_assembly1.sam"
end

file "results/#{@sample}/#{@sample}_illumina_aligned_to_assembly_sorted2.sorted.bam" => [ "results/#{@sample}/#{@sample}_pilon_polished_round1.fasta.1.bt2" , "#{@R1}" ] do
    puts "Aligning reads to pilon polished round1 fasta for CD156"
    sh "source bowtie2-2.2.9; source samtools-1.9; bowtie2 --threads 8 --qc-filter --no-unal -x results/#{@sample}/#{@sample}_pilon_polished_round1.fasta -U #{@R1}  -S results/#{@sample}/illumina_aligned_to_assembly2.sam && samtools view -b results/#{@sample}/illumina_aligned_to_assembly2.sam | samtools sort -o results/#{@sample}/illumina_aligned_to_assembly_sorted2.sorted.bam && ln -s results/#{@sample}/illumina_aligned_to_assembly_sorted2.sorted.bam results/#{@sample}/#{@sample}_illumina_aligned_to_assembly_sorted2.sorted.bam && samtools index results/#{@sample}/#{@sample}_illumina_aligned_to_assembly_sorted2.sorted.bam && rm results/#{@sample}/illumina_aligned_to_assembly2.sam"
end
